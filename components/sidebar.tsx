...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="md:hidden sticky top-0 z-20 w-full border-b border-slate-800 bg-slate-900 text-slate-100">
          ...
          <nav className="ml-auto flex items-center gap-2.5">
            {items.map(({ href, label, Icon }) => (
              <Link
                key={href}
                href={href}
                className={`inline-flex items-center gap-2.5 rounded-md px-3.5 py-3 text-sm transition-colors duration-200 hover:bg-slate-800 hover:text-white ${
            ...
            <button
              ...
              className="ml-1 inline-flex items-center rounded-md p-2.5 hover:bg-slate-800 transition-colors duration-200"
            >
              <svg
                viewBox="0 0 24 24"
                className="h-5 w-5"
      ...
      {/* Desktop sidebar */}
      <aside className="hidden md:flex md:w-72 md:min-h-screen md:flex-col md:border-r md:border-slate-800 md:bg-linear-to-b md:from-slate-700 md:to-slate-800 text-slate-100">
        <div className="p-5 h-26 flex items-center align-middle  bg-slate-700 border-b border-slate-600 mb-2">
          <Link href="/dashboard" aria-label="Αρχική">
            <Image
              src="/logowhite.webp"
              alt="Logo"
        ...
        <nav className="px-3 py-2 space-y-2.5 flex-1">
          {items.map(({ href, label, Icon }) => (
            <Link
              key={href}
              href={href}
              className={`group relative flex items-center gap-4 rounded-md px-5 py-3.5 text-[16px] transition-colors duration-200 ${
                pathname.startsWith(href)
        ...
        <div className="p-4 border-t border-slate-600 bg-slate-900 mt-auto">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <Link
                ...
                className="inline-flex items-center rounded-md p-2.5 hover:bg-slate-800 transition-colors duration-200"
